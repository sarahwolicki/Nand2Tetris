...
def trans_line(line: str, progname):
    ...
    elif command == 'goto' or command == 'if-goto' or command == 'label':
        label_name = splitline[1]
        out = control.generate(command, label_name, progname)
    elif command == 'call' or command == 'function':
        function_name = splitline[1]
        n = splitline[2]
        out = function.generate(command, function_name, n)
    elif command == 'return':
        out = function.gen_return()
    else:
